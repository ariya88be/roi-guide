...
- `lib/pins/query.ts` — **server-only** PostGIS pin query (GiST bbox) + property-detail query. Recomputes cash flow per listing from the caller's assumptions via the ROI engine (makes sliders/all-cash live), colours via `lib/roi/color`. Detail re-runs the engine for the itemised breakdown.
- `lib/redis/client.ts`, `lib/ratelimit.ts` — ioredis client + fixed-window rate limiter (fail-open).
- `app/api/pins/route.ts`, `app/api/property/[id]/route.ts` — public read endpoints: rate-limited, Zod-validated, generic errors. Return GeoJSON / detail JSON.
- `app/components/MapView.tsx` — client MapLibre map: cash-flow pins (continuous gradient + $ labels), filter controls (target/budget/all-cash/colourblind), legend, click→detail card ("show the math"). Keyless OSM raster basemap for now (MapTiler/Protomaps vector tiles in production). No clustering yet (one ZIP ≈ 20 pins).
