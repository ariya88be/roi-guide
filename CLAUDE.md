...
- `app/components/MapView.tsx` — client MapLibre map: **teardrop map-pins** (an SDF icon via `makePinImage`/`addImage(sdf:true)`, tinted per-feature by the deal-quality colour) with the rank number in the head, + a heat layer, filter controls (target/budget/all-cash/houseOnly/colourblind/Night-mode/rank-toggle/**cluster-toggle**), a collapsible rank-ordered property list (also the QA §15.K screen-reader list alternative) with a per-property hide/show eye-icon toggle (hidden properties are removed from the map/ranking but stay in place in the list, greyed out), hover "sneak peek" popup, click→Zillow + detail card ("show the math"). Keyless CARTO raster basemap (Voyager/Dark Matter — swaps live via `setTiles`, guarded against the pre-load race). Dense areas cluster natively (MapLibre `cluster: true` on the "pins" source), coloured by average deal score and **graded by an ordinal A+…Z- rank** across the clusters in view (78 unique grades, best = A+, surplus → Z-) rendered as HTML markers over the bubbles (an ordinal rank can't live in a MapLibre expression); click-to-zoom via `getClusterExpansionZoom`; the cluster toggle flips `setClusterOptions({cluster})` to show every pin individually. ⚠️ Heavy successive edits can wedge Turbopack HMR (map appears broken / stale bundle) — `rm -rf .next` + restart, it's not a code bug.
